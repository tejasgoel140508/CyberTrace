...
CyberTrace is a local cybersecurity threat-intelligence and investigation prototype built for a hackathon. It turns an indicator of compromise (IOC) into transparent evidence, correlation, Attack DNA, associated infrastructure context, a persistent analyst case, and a shareable investigation report.
...
`IOC → validation → enrichment → normalization → risk scoring → correlation → Attack DNA → graph → associated infrastructure map → persistent case → report`
