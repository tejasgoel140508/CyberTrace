...
Endpoints: `GET /dashboard/stats`, `GET /iocs?search&type&severity&page&limit`, `GET /iocs/:id`, `POST /iocs/search` (`{value}`), `GET /iocs/:id/relationships`, `GET /attack-dna`, `GET /attack-dna/:id`, `GET /map/threats`, `GET /map/country/:country`, `GET /investigate/:ioc`, `GET /cases`, `POST /cases` (`{iocId,title?}`), `PATCH /cases/:id` (`{title?,status?,verdict?,notes?}`), `GET /reports`, `POST /reports` (`{iocId?,attackProfileId?,title?}`), and `GET /reports/:id`.
